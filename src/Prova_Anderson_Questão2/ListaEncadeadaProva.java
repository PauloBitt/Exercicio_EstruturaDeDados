...
public class ListaEncadeadaProva<T> {
    ...
    public ListaEncadeadaProva(){
        ...
        public void organizaQuartos(){
            for (int i = 0; i < totalDeElementos; i++){
                for(int j = i+1; i < j; j++){
                    Celula now = new Celula();
                    Celula after = new Celula();
                    T now1 = Recupera(i);
                    T after1 = Recupera(j);
                    now.setElemento(now1);
                    after.setElemento(after1);
                    after.setProxima(now);
