 ...
 * @author paulo
 */
public class ListaDuplaProva<T> {
    ...
    public ListaDuplaProva(){
        this.primeira = null;
        this.ultima = null;
        this.totalDeElementos = 0;
    }
    
        public void trocarPosicao(int pos){
            if (totalDeElementos == 1){
                throw new ArrayIndexOutOfBoundsException("A lista só possue um elemento!");
            } else if(totalDeElementos == 0){
                throw new ArrayIndexOutOfBoundsException("A lista está vazia!");
            } else if(pos == tamanho()-1){
                throw new ArrayIndexOutOfBoundsException("A posição selecionada é a ultima da lista, então não irá mudar em nada, por favor escolha outra posição");
            } else if(pos > totalDeElementos){
                throw new ArrayIndexOutOfBoundsException("Posição Invalida!");
            } else{
                T valor = Recupera(pos);
                adicionaFim(valor);
                removerPosicao(pos);
            }
        }
